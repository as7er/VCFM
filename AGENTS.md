...
> 缓存：**vcfm-v149**（球队可读短名；前序 v148 球探属性标签辨识度）
...
- 缓存 **vcfm-v149**
- **球队可读短名 v149**：188 队中英文城市/地区短名分别全局唯一；界面主短名由机器四字母代码改为本地化可读名称（如 `西湾 / Westbay`），随语言切换同步更新并用于赛程、榜单、比赛事件、新闻及转会；四字母 `shortName` 仅保留给内部唯一代码和队徽字母，窄列悬停仍可查看完整队名
