...
> 缓存：**vcfm-v156**（审计对齐 198 队、SW 补 matchview 子模块、短名去重）

## v156 健康修复（2026-07-27）

- 审计脚本对齐 v152 赛制：11 联赛 × 18 队 = **198** 俱乐部、联赛赛程 3366、能力配额按一线队人口相对 3572 基准缩放
- `competition-audit` 门将榜改为 limit 内非空断言，消除 4 场抽样下的脆弱 `=== 8`
- `sw.js` 预缓存 `matchview-fsm/coords/director.js`，避免离线直播层缺模块
- 西乙「金丘联盟」更名为「金岭联盟」，恢复中文城市短名全局唯一
- 清除 `matchview.js` 生产路径 DEBUG 日志；`saveGame`/`setActiveSlot` 槽位钳制到 1–3
- 缓存与入口资源版本同步为 `vcfm-v156`
...
- **球队可读短名 v149 / v156**：198 队（v152 起 11×18）中英文城市/地区短名分别全局唯一；界面主短名由机器四字母代码改为本地化可读名称（如 `西湾 / Westbay`），随语言切换同步更新并用于赛程、榜单、比赛事件、新闻及转会；四字母 `shortName` 仅保留给内部唯一代码和队徽字母，窄列悬停仍可查看完整队名
...
- **能力稀缺标尺 v144 / v156**：以 3572 人一线队为基准约 2 名 OVR 20、24 名 19、110 名 18；人口变化时按比例缩放（198 队约 3762 人 → 约 2/25/116）；位置生成已归一化，潜力、身价和工资同步更新，国家队与俱乐部继续共享同一球员对象与能力
